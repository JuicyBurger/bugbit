...
async function runAgent(apiKey, model, prompt, cwd, customTools) {
    ...
    try {
        ...
        try {
            ...
            const result = await Promise.race([
                run.wait(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Agent timed out after 45 minutes')), AGENT_TIMEOUT_MS)),
            ]);
