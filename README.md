...
| `auto-describe` | no | `false` | Run an agent pass that appends a Qodo-style section (type, bullets, mermaid, file walkthrough, test plan) after the developer's existing PR body. Runs once per PR (skipped if the auto-describe markers are already present); review still runs on later pushes |
